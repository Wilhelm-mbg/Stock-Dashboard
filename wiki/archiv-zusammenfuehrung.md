...
8. **Skalenkonvention (offen, 03.09. abends):** Die Prüfung des Nachholers deckte auf, dass Yahoo-Intraday split- und spin-off-bereinigt ist, Alpaca-raw nicht. Folgen: (a) MNST/SPGI-Alpaca-Bereiche sind falsch skaliert und werden ersetzt (Alpaca-Bereiche über `quellen` identifizierbar, neu holen mit der Bereinigung, die Yahoo entspricht — empirisch prüfen: `adjustment=split` gegen Yahoo bei MNST UND SPGI); (b) die Vollsammlung braucht eine Konvention: **roh + Kapitalmaßnahmen-Tabelle** (append-only, jede Bereinigung ableitbar) oder bereinigt (Yahoo-Konvention, aber jede künftige Maßnahme ändert die ganze Historie). **Entscheid Wilhelm (03.09. abends, Formular): BEIDES** — Vollsammlung roh (`alpaca1m/`) + Kapitalmaßnahmen-Tabelle (Alpaca corporate actions) + bereinigte Kopie (`alpaca1m-bereinigt/`), die lokal aus beidem abgeleitet wird (kein zweiter Abruf; bei neuer Maßnahme nur der betroffene Wert neu abgeleitet). Für die Yahoo-Mischdateien (Nachholer) gilt die Yahoo-Konvention: MNST/SPGI-Alpaca-Bereiche werden ersetzt (erledigt 03.09.). *Nachtrag 03.09. spät:* Abspaltungen liefern in der Maßnahmen-Tabelle keinen Kursfaktor (Z1c-Befund) — **Wilhelm erlaubt den Zweitabruf** je betroffenem Wert (Faktor = `all ÷ dividend`, Herkunft `gemessen`), Auftrag folgt nach Phase M. Hinweis: die Yahoo-Dateien selbst haben dasselbe Problem schon vor Alpaca — „Datei gewinnt" hält nach einem Split die alten Kerzen auf alter Skala, nur die neu geholten sind umgerechnet.
   ...
   ***(d) ERLEDIGT 04.09.2026 (Bau) — die Lücke ist zu 54 % geschlossen, und der Rest ist benannt.*** Wilhelm hat den Zweitabruf erlaubt; `tools/alpaca-abspaltungsfaktor.js` misst den Kursfaktor als Median `dividend ÷ all` über 20 Handelstage vor dem Wirkungstag (Regel, Kriterien und Zahlen in [datenquellen.md](datenquellen.md)). **406 Abrufe, 201 Abspaltungen in 177 Werten → 109 Kursfaktoren in 108 Werten, 92 unklar.** Beide Eichungen bestanden: SPGI **1,057244** gegen die unabhängige Messung der Skalenreparatur (1,057), Placebo AAPL exakt **1,000000**. **Die Richtung ist nachgerechnet:** `all ÷ dividend` ist 0,9459, der Kursfaktor sein Kehrwert 1,0572 — der Satz oben („ergäbe 1,0572") meint den Faktor, nicht das Verhältnis. **Ein Fund, der die Sperre erzwungen hat:** 9 von 201 Werten tragen am Wirkungstag eine ZWEITE faktortragende Maßnahme (MHUA: Zusammenlegung 100:1 **und** Abspaltung am selben Tag). Der gemessene Faktor trägt dann beide, und die Ableitung hätte den Split ein zweites Mal angewandt — bei MHUA durch 0,0001 statt durch 0,01, hundertfach daneben und in jeder Zusammenfassung unauffällig. Sie bleiben „unklar"; `--pruefen` sucht sie ohne Abruf über alles schon Geschriebene (vier standen dort bereits, bevor die Sperre gebaut war). **Die Kontrolle, die (b)+(c) für SPGI nicht bestehen konnte, besteht jetzt:** bereinigt/Yahoo **0 von 64 Tagen** außerhalb 0,999–1,001, während roh/Yahoo an den 20 Tagen davor die 1,057000 zeigt. `--ableiten` ist für die 78 betroffenen Werte gefahren, deren Rohjahre schon liegen; die übrigen 30 folgen mit dem Vollauf. **Auch ein Teillauf pflegt jetzt `_regel.json`** — vorher hätte die Liste „ohne Kopie" nach dem ersten Teillauf weiter behauptet, ein längst bereinigter Wert habe keine Kopie.
