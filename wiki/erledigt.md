---
tags: [steuerung]
---
# Erledigt

*Abgeschlossene Aufträge, eine Zeile je Auftrag, mit Fundstelle. Was daraus weitergilt, steht auf
der Fachseite (Belegstand, Kosten, Datenquellen, Oberfläche, Archiv-Zusammenführung). Diese Liste
ist nur das Register, damit [offene-auftraege.md](offene-auftraege.md) offen bleibt.*

## September 2026

| Datum | Auftrag | Ergebnis in einem Satz | Fundstelle |
|---|---|---|---|
| 07.09. | Anhang an Ort und Stelle + Live-Menge alle (Nr. 23) | Journal statt Temp-Kopie: Schreibvolumen je Runde 8 GB → 156 MB, Reparatur byteidentisch nach Abbruch an drei Stellen; Live-Menge 3.067 geführte Reihen aus der Lebenszeit; Suite 4.680 → 4.744 | `uebergabe/anhang-an-ort-2026-09-07.md`, 23e55ae |
| 07.09. | QS Live-Sammler + Nachlauf (Nr. 24, nur lesen) | 38 Befunde (1/11/26 nach Schwere), Dienstag-Blocker Ruhe-Regel, Sperr-Race reproduziert, Kette/Manifest/Lücken/Lebenszeit unabhängig bestätigt; Proben je Punkt liegen bei | `uebergabe/qs-live-sammler-2026-09-06.md` + `-anhang/` |
| 06./07.09. | Nacharbeiten zur Vollsammlung (Nr. 6) | `--nachholen` (Tagesnachlauf ab letztem Stempel), `--manifest` (SHA-256 je Datei), Lückenliste (3,25 % fehlend, Verschwundene 8,0 %), `--ableiten` alle (7.184 Kopien), Komma-Falle behoben; Suite 4.636 → 4.680; abgenommen mit Auflagen F7/F8 | `uebergabe/vollsammlung-nacharbeiten-2026-09-06.md`, a8b7d44 + 7d472e7 |
| 06.09. | Live-Sammler über Alpaca (Nr. 4, Teil 1 des Sammelauftrags) | Minutenarchiv läuft während der Sitzung mit: alle 5 Min SIP-Balken bis jetzt−16 min für top500 + Watchlist + Positionen + Viewer-Wert, eine additive Schreibroutine für App und Werkzeug, Viewer 5m/15m/1h aus Alpaca-Minuten; Suite 4.505 → 4.636, 17 Gegenproben; Release fällig | `uebergabe/live-sammler-2026-09-06.md`, 91a6abf |
| 05.09. | Alpaca-Vollsammlung Z1c — der Vollauf | 47.419 Dateien, 2,62 Mrd Minutenkerzen, 120,6 GB, 0 Fehler, 2016–2026, Universum + 5.082 Verschwundene; erstes Archiv ohne Überlebensverzerrung | `E:/Markt-Dashboard-Archiv/alpaca1m/_fortschritt.json`, `_lauf.log`, [archiv-zusammenfuehrung.md](archiv-zusammenfuehrung.md) §5 |
| 05.09. | Viewer 8c — Zeichenwerkzeuge | 14 Werkzeuge, reines Modul in Daten-Koordinaten, Fibonacci, Messbox, Magnet, Store je Kürzel; 74 Klinken, 7 Gegenproben; 300k/350k | `uebergabe/viewer-8c-2026-09-05.md` |
| 05.09. | Viewer 8b — Chart-Einstellungen | Dialog mit vier Reitern über den Dialogstapel, reines Modul mit 41 Feldern, 0 tote Felder, Farbwechsel am Canvas gemessen, Kontrast beider Themen; 273 Klinken, 15 Gegenproben; 205k/300k | `uebergabe/viewer-8b-2026-09-05.md` |
| 05.09. | Viewer 8a — Reparatur und Grundbedienung | Zeitraum × Kerze, Maus-Zoom, MA über die ganze Reihe (260/260 statt 61/260), Signale im Kerzenchart, Fußzeile ein Satz; 75 Klinken, 10 Gegenproben; 215k/300k | `uebergabe/viewer-8a-2026-09-05.md` |
| 04.09. | Oberfläche Stufe 7 — Schein-Finder | Zwei Karten, acht Listen mit Bereichen, drei gemessene Voreinstellungen, Live-Filter, sieben Spalten + Schalter, Stufen-Pillen; reines Modul `scheinwahl.js`; 98 Klinken, 8 Gegenproben; 210k/250k | `uebergabe/scheinfinder-ui-2026-09-04.md` |
| 04.09. | Belegwort, Einheiten, verwaister Eintrag, Nachbilden (FA2/U4/U5/U6/FA1) | „belegt" → „gemessen" + Belegwort-Sperrklinke über alle Module (0 Fehlalarme); Nachbilden-Knopf nur bei Scheinen; 16 → 9 Blöcke > 240; 13 Gegenproben; 370k/200k | `uebergabe/belegwort-nachbilden-2026-09-04.md` |
| 04.09. | Dialoge in Ordnung (B1/U1/U2) | `dialogstapel.js`, Rückweg je Dialog, Titel 1.240 → 21; Dialog-Sonde repariert und mit Ketten + Positivkontrolle: 8 → 0 Befunde; 285k/250k | `uebergabe/dialoge-2026-09-04.md` |
| 04.09. | Laufband, Marktglocke, Kleinkram (FA3/U3/U7 + 2 PM-Funde) | Band läuft 60 px/s trotz „Bewegung reduzieren" (App-Schalter), jüngste 20 mit Alter; Glocke 09:30/16:00 ET synthetisch mit Probe-Knopf; 14 Gegenproben; 350k/200k | `uebergabe/laufband-glocke-kleinkram-2026-09-04.md` |
| 04.09. | Vollständige UI-QS v8.40.2 (fünf Agenten) | 7 von 12 Altfunden behoben; neu 1 blockiert (Dialoge), 3 falsch (NaN-Ticket, „belegt", Kontrast), 7 unschön; 1,89 Mio Token gegen 600k | `uebergabe/ui-qs-voll-2026-09-04.md` |
| 04.09. | Oberfläche Stufe 6 — Aktien-Viewer | Kerzenchart als reines Modul, Leseauskunft `archiv-kerzen`, Vor-/Nachbörse abgesetzt, Kursabruf zusammengelegt, F2/F4/F6/F7/F8 behoben; 151 Klinken, 28 Gegenproben; 445k/300k | `uebergabe/oberflaeche-stufe6-2026-09-04.md`, [oberflaeche.md](oberflaeche.md) |
| 04.09. | Texte und Zählungen (QS-F3, F5, F9, F10, F11) | `#subnote` weg; Satz 8 → 5 in der App, Zusicherung an der App; zwei Textwände hinter i-Knöpfe; Dauertext-Abtaster an der Laufzeit; 47 Klinken, 4 Gegenproben rot, eine geschärft; 330–380k/150k | `uebergabe/texte-zaehlungen-2026-09-04.md` |
| 04.09. | Prüfwerkzeuge schärfen (QS-F12, S1–S9) | `tools/ui-struktur.js` liest die laufende Instanz; a11y mit Kunstdaten; echte Tab-Wanderung mit bekannten Abweichungen; F6 anders reproduziert (Verschiebung statt Lage); 29 Gegenproben rot; 305k/150k | `uebergabe/pruefwerkzeuge-2026-09-04.md` |
| 04.09. | Rumpf bei 1024 px (QS-F1) | Positionstabelle 9 Spalten + zweite Zeile + Scroll-Kasten; 1040 → 1014 px; Bildlauf-Klinke in ui-probe/ui-aufnahmen; 18 Gegenproben rot; 291k/100k | `uebergabe/rumpf-1024-2026-09-04.md` |
| 04.09. | UI-QS v8.40.0 | 12 Funde (1 blockiert, 4 falsch, 7 unschön), 0 tote Bedienelemente von 294, Kontraste > 4,5 in beiden Themen; Urteil zur Struktur-Seite: Landkarte ja, Inventar nein | `uebergabe/ui-qs-2026-09-04.md`, `uebergabe/aufnahmen-qs-2026-09-04/` |
| 04.09. | Universum mitliefern (#111) | Eingefrorene Liste im Paket, Erststart-Kopie schreibgeschützt, nie überschreiben; 22 Klinken, 21 Gegenproben rot (2 geschärft, 1 Absturz → 6 rote Zeilen); 180k statt 60k | `uebergabe/universum-mitliefern-2026-09-04.md` |
| 04.09. | Sammler verhungert nicht mehr + Markt-Takt 1 Min + Grundmenge alle Stammdaten | Leerer Versuch als `versucht` gemerkt, `reihenfolge()` + neues Modul `sammelrunde.js` (alle fälligen Intervalle je Blick), Stillstandsbremse, Yahoo-Leerblöcke sichtbar; Grundmenge 1.653 (SEC-Stammdaten, nicht 3.263); 31 Gegenproben rot, drei Klinken geschärft; 395k statt 300k | `uebergabe/sammler-verhungern-2026-09-04.md`, [fehlerformen.md](fehlerformen.md) |
| 04.09. | Oberfläche Stufe 5 — Reiter „Markt" | Vierter Reiter, drei Pillen, reines Modul `markt/uebersicht.js`, zwei Leseauskünfte, 171 Zusicherungen; v8.39.0 | `uebergabe/oberflaeche-stufe5-2026-09-04.md`, [oberflaeche.md](oberflaeche.md) |
| 04.09. | Kursfaktor für Abspaltungen (Zweitabruf) | 109 Kursfaktoren in 108 Werten, 92 unklar; SPGI 1,057244 gegen unabhängige Messung; Doppel-Maßnahmen am Wirkungstag gesperrt | `uebergabe/abspaltungsfaktoren-2026-09-04.md`, [datenquellen.md](datenquellen.md) |
| 03.09. | Z1c Alpaca-Vollsammlung — Werkzeug | Gebaut, Testlauf bestanden, Phase L und M gefahren (8.055 Werte führt die Quelle, 177 ohne Kursfaktor); Vollauf läuft seit 03.09. 23:04 | `studien/alpaca-vollsammlung-2026-09/BEFUND.md`, [archiv-zusammenfuehrung.md](archiv-zusammenfuehrung.md) §5 |
| 03.09. | Z1 Skalenreparatur MNST/SPGI | Keine Alpaca-Bereinigung entspricht Yahoo; 46.955 Kerzen mit gemessenen Faktoren ersetzt, Sicherung vorher | `uebergabe/skalenreparatur-2026-09-03.md`, [datenquellen.md](datenquellen.md) |
| 03.09. | Z1 Nachholer gefahren (PM) | 1.507 Aufgaben, 2.954.499 Balken, 0 Fehler; Prüfung 5/70 → nach Reparatur 3/70 Rauschen | `studien/archiv-zusammenfuehrung-2026-09/nachholer-*.log` |
| 03.09. | Z1 Migration geschrieben (PM) | 653 Dateien, 34 neu, 434.869/257.060/78.413/144.827 Kerzen übernommen, Rasterverlust 0, Äquivalenz wie Trockenlauf | `…/migration-schreiben-2026-09-03.log` |
| 03.09. | Alpaca-Balken-Probe + Freigabe | 7 von 8 Kriterien; Wilhelm: Freigabe mit Nachtrag (ARM 9/383 > 0,1 %) | `…/probe-alpaca-balken-ergebnis.json`, [entscheide.md](entscheide.md) |
| 03.09. | Rasterfilter-Fix R5 | Minute-0-Regel nur noch 60m-Nicht-Krypto; Trockenlauf-Verlust 0/0/3/0 | `uebergabe/rasterfilter-fix-2026-09-03.md` |
| 03.09. | Z1 Format 2, Migrationswerkzeug, Nachholer | Quellenpflicht je Kerze, Trockenlauf über 799 Reihen | `uebergabe/archiv-z1-2026-09-03.md`, [archiv-zusammenfuehrung.md](archiv-zusammenfuehrung.md) §8 |
| 03.09. | Oberfläche Stufe 4 — Archiv-Grafik, Kopfzeile | Ein Balken je Auflösung über 60 Handelstage, Kopfzeile einzeilig (67 → 33 px); zwei Klinken blind gefunden | `uebergabe/oberflaeche-stufe4-2026-09-03.md` |
| 03.09. | Oberfläche Stufe 4b — Mittelfrist in den Maschinenraum | Zwölfte Klappe, alle 23 Kennungen erhalten, Einstellungen −47 % Text; K7-Fund | `uebergabe/oberflaeche-stufe4b-2026-09-03.md` |
| 03.09. | Oberfläche Stufe 3 — Schnitt | 36 Sätze ins Erklärregister, Dauertext −17 %, Widerlegtes ins Archiv | `uebergabe/oberflaeche-stufe3-2026-09-03.md` |
| 03.09. | Oberfläche Stufe 2 — Bestand & Kopfzeile | Drei Buch-Karten, ein Verlauf, elf Statuszeilen im Maschinenraum | `uebergabe/oberflaeche-stufe2-2026-09-03.md` |
| 03.09. | Historische Spannen-Studie | Kassa-Hürde je Umsatzklasse gemessen (0,157 / 0,085 / 0,065 / 0,045 Pp), Cent-Boden, 2 von 31 Varianten wieder offen | `uebergabe/spannen-auswertung-2026-09-03.md`, [kosten.md](kosten.md) |
| 03.09. | Zusatz C, Übernacht-Nachtrag, Zusatz A ohne Gegenüberstellung | Verschwundene Werte: Aufschlag vierte Stelle; Schluss-Hürde mit Band; Buch-Hürde Faktor 9,1 über Korbspanne | `studien/vorregistrierung-2026-09-02-spannen-historisch/ERGEBNIS-*.md` |
| 02.09. | Oberfläche Stufe 1 — Umzug auf drei Bildschirme | 3 Reiter, 9 Pillen, elf Klappen, 36 Klinken umgeschrieben, 37 Gegenproben rot | `uebergabe/oberflaeche-stufe1-2026-09-02.md` |
| 02.09. | Momentum-Buch auf liquiden Korb | Buch = gemessene Konfiguration, Block 34 hält es gegen `lauf-*.json` | `uebergabe/momentum-buch-liquide-2026-09-02.md` |
| 02.09. | Momentum liquide gemessen | „LEBT" nach Regel, In-Sample, am Rand (untere Grenze +0,05, 18/63 Lagen) | `studien/vorregistrierung-2026-09-02-momentum-liquide/ERGEBNIS.md`, [belegstand.md](belegstand.md) |
| 02.09. | Monats-Momentum am CFD gemessen | „nicht entscheidbar", Obergrenze netto +0,605 Pp je Umlauf | `studien/vorregistrierung-2026-09-02-momentum-messung/ERGEBNIS.md` |
| 02.09. | Wiedervorlage klein | 31 von 52 Varianten gegen CFD geschlossen; Depot-Kandidatenliste NEIN | `studien/wiedervorlage-2026-09-02/BERICHT.md` |
| 02.09. | Aktien-Kostenmessung am Alpaca-Paper gebaut | `alpaca.js` als zweites Gefäß; Aktivierung ist Wilhelms Klick | `uebergabe/alpaca-kostenmessung-2026-09-02.md`, [kosten.md](kosten.md) |
| 01.09. | News-Sentiment gratis gemessen | NEIN, kein Blindbefund | `studien/vorregistrierung-2026-09-01-news-sentiment-vollkorpus/ERGEBNIS.md` |
| — | Konservativitäts-Audit | GESTRICHEN: Messgenauigkeit ist nicht das Problem, die Kantengröße ist es | [entscheide.md](entscheide.md) |
| — | Insider-Käufe (Form 4) | Weggelassen (Entscheid 01.09.) | [entscheide.md](entscheide.md) |
