...
| **5 Reiter „Markt" — reines Markt-Dashboard** | Vierter Reiter `data-tab="markt"` mit drei Pillen: **Überblick** (Index-Kacheln mit Tagesverlauf inkl. Gold/Öl/Dollar, Sektor-Leiste 1T/1W/1M aus `markt/stammdaten.json` + Kursen, Hotlists Gewinner/Verlierer/meist gehandelt/ungewöhnliches Volumen aus dem Archiv-Median, Earnings heute/morgen, Vor-/Nachbörse-Zustand, 3–5 Schlagzeilen), **Marktkarte** (zieht um), **Radar** (Spekulationen, Insider, Vorbörse — zieht um). Aus Heute→Überblick wandern `#tiles`, `#dashHeat`, `#bigtech`/`#chips`, `#news`, `#newsTicker`, `#calendar` nach Markt; Heute→Überblick behält Bestand (drei Buch-Karten, Zuletzt getan, Verlauf). IDs bleiben, DOM-Blöcke wandern. `yahoo-quotes` (main.js) um Volumen/MKap-Felder erweitern. | **beauftragt 04.09.** (Wilhelm: TradingView als Vorbild, „nicht so vermixt") — Stufe 6 = Aktien-Viewer |
