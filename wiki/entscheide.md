...
- **Viewer nach TradingView-Muster, drei Stufen** (05.09., Wilhelm, Formular nach Screenshots der TradingView-Einstellungen und Werkzeugleiste): 8a Reparatur und Grundbedienung, **8b Einstellungen-Dialog JA**, **8c Zeichenwerkzeuge JA**, 8d Alarme NEIN (vorerst); **Kauf/Verkauf-Knöpfe im Chart NEIN** — der Viewer bewertet nichts und handelt nichts (bestätigt). Nicht gebaut werden Elliott, harmonische Muster, Gann, Ideen, Logo. Aufträge Nr. 20–22.
- **Glocke klingt wie die NYSE-Börsenglocke** (05.09., Wilhelm, per Video-Link): elektrische Klingel über ~10 s statt eines Kirchenglocken-Anschlags, zum Schluss der Hammer; synthetisch erzeugt, keine fremde Aufnahme. Auftrag Nr. 19.
- **„Trade nachbilden" auf Basiswert-Positionen** (04.09., Wilhelm, Formular zu QS-FA1): Knopf „Nachbilden" wird bei Basiswert-Positionen ausgeblendet (an einer Aktienposition ist nichts nachzubilden). **Schlagzeilen-Laufband** (04.09., Formular zu Stufe-6-Vorschlag 1): Laufband bleibt als Breaking-News-Band mit den jüngsten Meldungen und läuft wieder von rechts nach links — auch bei „Bewegung reduzieren" (eigener App-Schalter); NEU: Glocke zu Marktöffnung und -schluss („nettes Feature"). Beides in [offene-auftraege.md](offene-auftraege.md) Nr. 16/17.
...
- **Live-Sammler: Live-Menge × Kopiervolumen** (06.09., aus der Übergabe zu Nr. 4, Fragen 1 und 4): heute top500 + Watchlist + Positionen + Viewer-Wert; der Anhang kopiert die Jahresdatei (Temp + rename) — ≈ 1,9 GB je Runde × 80 Runden am Tag, und `E:` ist die WD-**HDD**. „Alle 3.232" hieße ~17 Blöcke je Runde und 6× Kopiervolumen — nur mit Anhang an Ort und Stelle + Reparaturjournal tragbar (Folgeauftrag ~150k). **Entschieden 06.09. 23:15 (Wilhelm, Formular): „Alle 3.232 Aktien, Folgeauftrag Anhang an Ort und Stelle"** — gegen die PM-Empfehlung (eine Woche top500 messen). Folge: Auftrag Nr. 23 (`uebergabe/auftrag-anhang-an-ort-2026-09-07.md`), erst der Umbau, dann die Menge; beides vor dem Release. Reihenfolge morgen (zweite Frage): Glocke-Chat vor der Wache — mit Nr. 23 zum Sammelauftrag 07.09. gebündelt, Daten vor der Glocke.
- **Signalstudie auf Minutenbasis, vier Auslegungen** (PM 07.09. 08:05, an die Sitzung geschickt und hier abgelegt; **vor** der ersten Urteilszahl, nichts Vorregistriertes geändert — der Chat trägt sie als Nachtrag 1a nach): **(1) Aufteilung des Vollaufs** nach Zeitrahmen, nie nach Umsatzklasse — die Klasse kommt im Urteil vor, eine Auswahl darauf wäre ein Eingriff; acht Teile à ≤ 12 h, der PM legt die Aufgaben an. **(2) 80-%-Regel bleibt**: dass die Klasse 5–50 Mio $ auf 1m herausfällt, ist ein Befund über dünne Minutenreihen, kein Mangel der Regel; Reihen und Reihen-Tage je Zeitrahmen × Klasse ausweisen, damit ein Nullbefund dort als „nicht gemessen" lesbar bleibt. **(3) Endpunkt** bleibt der rohe Netto-Ertrag; marktbereinigt und Einstiegsfenster stufen herab — „belegt" ≠ „handelbar", und die Spalte „handelbar" ist die, die Wilhelm liest. **(4) Short nie „handelbar"**: die Leihe ist nicht gemessen (Kosten unbekannt, bei illiquiden und verschwundenen Werten oft gar nicht verfügbar) — keinen Leihsatz erfinden, Vermerk „Leihe nicht gemessen", höchstens „belegt als Größe".
- **Signalstudie, drei weitere Auslegungen** (PM 07.09. 11:30, an die Sitzung geschickt und hier abgelegt): **(5) fehlende Jahresdateien** sind kein Auftrag an die Vollsammlung — nachgemessen, 340 von 340 in der Lückenliste erfasst ([messmethodik.md](messmethodik.md)). **(6) Pflanzform der Positivkontrolle abgenommen** (Kante in der Eröffnung der Ausstiegskerze; die Block-Pflanzung erzeugt neue Signale und misst etwas anderes — bleibt nachrichtlich). **(7) Cent-Boden vor dem Vollauf beziffern:** Σ Einstiegskurs **und** ein Zähler der Signale, deren Cent-Boden über der Klassenhürde liegt — 80 MB und ein Pilot jetzt gegen 458 Prozess-Stunden noch einmal später; ohne den Zähler sagt der Mittelwert nichts über die Fälle, auf die es ankommt.
- **Signalstudie 1m: messen — entschieden 08.09. 17:10 (Wilhelm, Formular: „Ja, jetzt durchlaufen lassen").** Ursprüngliche Frage: 90 Konfigurationen, ≈ 369 Prozessstunden = 2,5 Tage in acht Teilen (BelowNormal), tagsüber neben dem Live-Sammler auf derselben Platte (Leselast klein, CPU halb). Prior aus 5m/15m und August stark negativ; es ist aber die letzte offene Frage der Familie, und das Messgerät steht. Empfehlung des PM: **ja, nachts-und-tags durchlaufen lassen**, weil die Frage danach endgültig beantwortet ist und ein späterer Lauf dieselben 2,5 Tage kostet.
- **Trendkanal auf Tagesbasis messen** (Wilhelm 08.09. ~17:20, im Chat: „ja, man muss ja warten bis sicher der Kanal ausbaut, das ist eher etwas, was man über Tage/Wochen laufen hat und nicht Minuten") → Auftrag Nr. 28. Zwei Festlegungen des PM darin, vor der Messung: Hauptgröße ist der **Überschuss gegenüber dem Topf** (über Wochen misst der rohe Ertrag nur den Markt), und die Standardfehler laufen über **Blöcke von H Tagen** (überlappende Haltedauern). Erwartungen beider Seiten stehen in der Vorregistrierung.
- **Limit statt Marktorder** (Wilhelm 08.09.): als Variante Nr. 29 vorgeschlagen, noch nicht beauftragt; Reihenfolge: erst 1m-Lauf, dann Trendkanal-Aggregation, dann die Limit-Variante (alle drei lesen dieselbe Platte).
- **Live-Sammler ohne Bremse — Bauweise (PM 09.09., Festlegung; Wilhelm informiert, Einspruch vor dem Start):** die Live-Runde schreibt keine Jahresdateien mehr, sondern je Reihe eine Tagesablage `_live.jsonl` (append, kein fsync), das Jahr schreibt allein der Nachlauf 23:30. Grund: gemessen ~235 von 300 s Blockade des Hauptprozesses je Runde (Nr. 30); ein Kindprozess allein hätte die Oberfläche befreit, aber die Platte weiter Dreiviertel der Sitzung belegt und das Dezember-Wachstum des Schwanzes (Nr. 27a) nicht gelöst. Wilhelms Entscheid „Live-Menge ALLE" bleibt; der Anhang an Ort und Stelle bleibt dem Nachlauf.
- **Bis zum Release von Nr. 30: Live-Sammler aus** (Werkzeuge → Kursarchiv → Häkchen „Live-Sammler"). Kosten: keine fürs Archiv (Nachlauf holt den Tag, 08.09. 23:30 Rückgabe 0); der Viewer sieht Alpaca-Minuten erst abends, nachmittags gilt die Yahoo-Naht. Wilhelm klickt selbst — der PM fasst die laufende App nie an.
- **Trendkanal Tage, die sechs Fragen der Übergabe (PM 09.09.):** (1) Klasse bleibt am **Signaltag** wie registriert — die Trades ohne Topfzelle (Pilot 0,6 %) werden gezählt und ausgewiesen, kein Nachtrag 3 (keine Änderung nach dem ersten Pilotlauf ohne Not); (2) 15 % Tage ohne 16:00-Kerze: akzeptiert, je Klasse ausweisen plus **ein Vorbehaltssatz** im Bericht; (3) „ausgebaut" filtert nur 10 % der App-Abschnitte — an Wilhelm gemeldet, Definition bleibt registriert; (4) Cooldown-Zahl in die Lesart; (5) Placebo-Schranke 0,045 Pp absolut im Vollauf prüfen, bei Fall Kand − PlB lesen; (6) Reihenfolge und Teile: Wilhelms Formular 09.09. (Vorschlag PM: Aggregation zuerst, 4 Teile, dann 1m fortsetzen).
- **1m-Lauf: Fortsetzung** nach dem Abbruch 08.09. 20:42 (Abmelden) — Wilhelms Formular 09.09.; die Aufgaben wurden neu angelegt (Laufzeitgrenze 7 Tage). S4U/„unabhängig von der Anmeldung" braucht Adminrechte, die die PM-Shell nicht hat (0x80070005) — sie bleiben interaktiv; Regel: während der Läufe nicht abmelden, nicht ausschalten. Abmeldefest nur durch Wilhelm selbst (Aufgabenplanung, Kennwort).
- **Trendkanal auf Tagesbasis: gemessen, NEIN (09.09.2026, 10:04).** Konsequenz: der Abschnittskanal bleibt in der App reine Anzeige (wie seit 25.08.), kein Kanal-Einstieg auf Tages-, Stunden- oder Minutenbasis; eine Wiederholung nur mit neuer Registrierung (andere Linie, andere Bedingung). Nr. 29 (Limit-Variante der Minutenstudie) bleibt Vorschlag, Reihenfolge: nach dem 1m-Lauf.
