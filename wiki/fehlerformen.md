...
- **Die Gegenprobe, die nichts auslöst.** Eine Schein-Position nur in `positions`, nicht in `trades`: der Dialog öffnete gar nicht, die Sonde meldete pflichtschuldig „kein NaN" — grün aus dem falschen Grund. Ebenso `window.api.fetchText = …`: die Brücke ist schreibgeschützt, die Zuweisung verpufft ohne Fehler, der echte Abruf läuft ins Leere. Gesehen hat es nur ein **Zähler in der Attrappe** (0 Abrufe, wo einer sein musste). Regel: jede Attrappe zählt, und eine Gegenprobe belegt erst, wenn sie den Weg nachweislich betreten hat. *Fundstellen: `uebergabe/belegwort-nachbilden-2026-09-04.md` §1, `scheinfinder-ui-2026-09-04.md` §8.*
- **Die Sonde, die ihre Frage im Kommentar verlor.** `tools/dialog-probe.js` nannte sechs Prüfungen im Kopf und stellte fünf: ein nie geschlossener Kommentar fraß die Zeile `if (!drin) befunde.push(...)`. Sie lag so seit 27.08. im Repo und sah aus wie ein bestandener Test. *Fundstelle: `uebergabe/dialoge-2026-09-04.md` §2.*
- **Das Belegwort in anderer Wortstellung.** Die alte Klinke suchte „belegte Kante" (Adjektiv); FA2 sagte „Regeln wurden … belegt" (Verb) — dieselbe Behauptung, andere Stellung, Sperre ins Leere. Und eine Verneinung 90 Zeichen später („keine tragfähige Kante") darf die Behauptung davor nicht decken: **Satzglieder, nicht Sätze.** *Fundstelle: `uebergabe/belegwort-nachbilden-2026-09-04.md` §2.*
...
## Die Gegenprobe misst das Falsche (07.09.2026)
Der Nachbesserungs-Chat fand beim Nachfahren der QS-Proben **fünf**, die selbst nicht maßen, was sie behaupteten: eine Sperrprobe setzte ihr Merkmal, sobald das **Lesen** „frei" sagte, statt auf den Rückgabewert des **Setzens** zu schauen; eine Probensammlung lud eine **eingefrorene Kopie** des geprüften Moduls und maß deshalb nach jeder Änderung weiter den alten Stand; eine Quell-Attrappe lieferte dieselbe Seitenzahl unabhängig vom angefragten Fenster, wodurch jede Zeitscheibe unsichtbar blieb; eine Verlustzählung zählte alle Balken jedes Werts, der **irgendwann** ruhte, auch wenn er später wieder aufwachte; eine Prüfung suchte wörtlich nach einer Schreibweise, die es nicht mehr gibt, während die Eigenschaft unverändert galt. Lehre: **eine Gegenprobe ist selbst ein Messgerät und braucht ihre eigene Positivkontrolle.** Wer eine fremde Probe als „vorher rot / nachher grün" benutzt, prüft zuerst, ob sie am geänderten Stand überhaupt etwas sehen kann — sonst ist Grün nur die Abwesenheit von Messung. Siehe auch „Prüfwerkzeug hält leeren Fall für Normalfall".

## Die Dauer im Log ist eine Blockade der Oberfläche (08.09.2026)
Seit dem 07.09. stand in zwei Übergaben und in Nr. 27 „geschrieben 155 MB in 215 s je Runde" — gelesen als Dezember-Problem (der Schwanz wächst), nicht als das, was es ist: **die Live-Runde schreibt synchron im Electron-Hauptprozess, der auch das Fenster bedient.** Ergebnis „Markt-Dashboard (Keine Rückmeldung)" für ~235 von 300 Sekunden, jeden Handelstag von 15:30 bis 02:21. QS-Chat (11 Prüfpunkte), Code-Chat und PM haben die Zahl gesehen; Wilhelm hat sie gespürt. **Regel:** zu jeder Zeitangabe aus einem Log gehört die Frage „in welchem Prozess?" — alles Synchrone im Hauptprozess (`fs.*Sync`, `JSON.parse` großer Dateien, `createHash`) ist Oberflächenzeit. Abnahme einer Runde immer mit einer `Responding`-Sonde je Sekunde über mehrere Runden, nie nur mit der Logzeile. Gegenstück in der Studie: „Querschnitt misst den Tag" — auch dort war die Zahl richtig und die Lesart falsch.

## Abmelden beendet Aufgaben, die „nur bei Anmeldung" laufen (08.09.2026)
Die acht 1m-Aufgaben waren mit `schtasks /Create` ohne `/RU` angelegt — Anmeldetyp „Interaktiv": beim Abmelden des Benutzers bekommt jede Konsole CTRL_LOGOFF, node endet mit 0xC000013A, der Lauf ist tot und niemand merkt es (der Monitor des PM stirbt mit der Sitzung, siehe „Hintergrundlauf stirbt mit der Claude-App"). Ein Ausschalten mit Schnellstart sieht im Ereignisprotokoll genauso aus (1074 winlogon, 7002/7001, kein Boot-Ereignis). **Was nicht ging:** die Aufgaben als S4U („unabhängig von der Anmeldung", ohne Kennwort) neu zu registrieren — `Register-ScheduledTask -LogonType S4U` und `schtasks /RU … /NP` verlangen eine erhöhte Shell (0x80070005), die der PM nicht hat und nicht anfordert. **Regel bis auf Weiteres:** während eines Studienlaufs nicht abmelden, nicht ausschalten, kein Schnellstart-Herunterfahren; abmeldefest kann nur Wilhelm die Aufgaben stellen (Aufgabenplanung → „unabhängig von der Benutzeranmeldung", Kennwort). Läufe mit Schlüsseln (Nachlauf) blieben ohnehin interaktiv, weil S4U das Benutzerprofil nicht lädt. Vor jeder Aussage „der Lauf läuft" die Prozessliste und die Sicherungspunkte prüfen, nicht das Wiki.

## Absolute Schranke aus einer anderen Skala übernommen (09.09.2026)
Die Trendkanal-Studie registrierte für die gepoolten Placebos das Band der Minutenstudie: |t| < 3 **und** |Mittel| < 0,045 Pp. In der Minutenstudie liegt die Placebo-se bei 0,006–0,03 Pp, da ist 0,045 ein weites Band. Bei Haltedauern von 5–20 Tagen liegt die Placebo-se bei 0,04–0,16 Pp — die absolute Schranke sitzt **im Rauschen**, und ein sauberes Placebo verfehlt sie in der Mehrzahl der Fälle: 6 von 8 Bändern „gefallen", alle acht mit |t| ≤ 1,9. Der Kunst-Satz des Piloten hielt alle acht Bänder, weil sein Rauschen (0,7 %) und seine 40 Reihen ohne gemeinsame Tage eine viel kleinere se ergeben als der Markt. **Regel:** absolute Schranken nie aus einer Studie mit anderer Zeitskala kopieren; sie als Vielfaches der erwarteten se der eigenen Skala registrieren (oder nur das t-Kriterium), und die Positivkontrolle muss die Schranke mit einem Rauschsatz prüfen, der die Korrelation über gemeinsame Tage trägt (Querschnitt misst den Tag). Die Aussage der Studie hängt daran nicht: die Kandidaten selbst sind in der Bestätigung negativ und Placebo B (gleiche Klasse, gleicher Tag) steht je Zeile daneben.

## Die Antwortzeit-Sonde misst die Last des Nachbarn (09.09.2026)
Die UI-Probe zur Nr. 30 misst die längste Blockade des Hauptprozesses über einen 10-ms-Zeitgeber. Der erste Abnahmelauf des PM lief **parallel** zu `node test-v6.js` (ein Kern bei Normal-Priorität) und zu acht Studienprozessen (BelowNormal): Befund 154 ms, Soll < 100. Allein wiederholt: 16 ms, wie in der Übergabe. Ein Zeitgeber-Drift misst nicht nur eigenes synchrones Warten, sondern jede Verdrängung durch gleich- oder höherrangige Prozesse. **Regel:** Antwortzeit- und Drift-Proben nie parallel zu anderen CPU-Läufen des PM fahren; ein roter Befund ist erst nach einem Lauf allein ein Befund. Der Umkehrschluss gilt nicht: ein grüner Lauf unter Last wäre Beleg.

**Nachtrag 09.09. 22:14 — dritte Erscheinungsform:** ein geschlossenes Konsolenfenster tötet den Teil genauso (^C, 0xC000013A). Abhilfe ohne Adminrechte: die Aufgabe ruft `powershell.exe -NoProfile -WindowStyle Hidden -Command "& <nacht.cmd> k/8 1m; exit $LASTEXITCODE"` auf — kein Fenster, nichts zu schließen; Abmelden und Neustart bleiben tödlich. Ab jetzt Standard für Studienläufe.

## Der Aufgabenplaner hängt, die Prozesse laufen weiter (11.09.2026)
Der tägliche Nachlauf (Aufgabe 23:30) blieb am 10.09. aus, ohne Fehlermeldung: Dienst „Schedule" läuft, aber jede Abfrage (`schtasks /query`, `Get-ScheduledTask`) blockiert minutenlang, und der Trigger feuerte nicht. Die schon gestarteten Aufgaben (acht Studienprozesse) liefen unbeeinträchtigt weiter — der Dienst startet Prozesse, er hält sie nicht am Leben. Parallel meldete OneDrive alle zwei Minuten DCOM 10010 (FileCoAuth), vermutlich derselbe verklemmte COM/RPC-Unterbau. **Regel:** wenn eine geplante Aufgabe ohne Logzeile ausbleibt, zuerst mit Zeitlimit prüfen, ob der Aufgabenplaner überhaupt antwortet (`Start-Job { schtasks /query … }` + `Wait-Job -Timeout 40`); ein hängender Planer wird nur durch Neustart des Rechners (oder des Dienstes mit Adminrechten — killt laufende Aufgaben) gesund; deshalb erst nach dem Ende laufender Läufe neu starten und ausgefallene Läufe von Hand nachholen. Datenlogik trägt: der Nachlauf holt ab dem letzten Stempel, ein ausgefallener Tag wird nachgeholt.

## Die QS zählte die eigenen Skripte, nicht die Module (12.09.2026)
Nach der Junction-Reparatur vom 07.09. (`npm install` nach gelöschtem `node_modules/.bin`) stimmte npms Sicht auf den Baum nicht mehr mit dem Inhalt überein: `npm ls --omit=dev` führte `electron-updater` ohne Kinder. electron-builder packt, was npm für Produktion hält — die an die Wurzel gehobenen Abhängigkeiten (`graceful-fs`, `builder-util-runtime`, `lazy-val`, `js-yaml`, `lodash.isequal`) fehlten im `app.asar` von 8.44.0 (und wohl 8.44.1). Die App lief, nur der Updater lud nicht — und genau der hätte den Fix verteilt. Die Release-QS prüfte „alle 57 Skripte im asar" (eigene Dateien) und war grün. **Regel:** Paket-QS heißt Produktionsbaum gegen Archiv, jedes Fremdmodul mit `package.json`, plus ein `require` der kritischen Module aus dem gebauten Paket; ein reparierter Baum gilt erst nach `npm ci` als heil. Dritte Erscheinungsform von „Build lieferte Module nicht aus" (Momentum 8.2x, Signaturblock 8.26).
