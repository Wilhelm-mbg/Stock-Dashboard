...
Die acht 1m-Aufgaben waren mit `schtasks /Create` ohne `/RU` angelegt — Anmeldetyp „Interaktiv": beim Abmelden des Benutzers bekommt jede Konsole CTRL_LOGOFF, node endet mit 0xC000013A, der Lauf ist tot und niemand merkt es (der Monitor des PM stirbt mit der Sitzung, siehe „Hintergrundlauf stirbt mit der Claude-App"). Ein Ausschalten mit Schnellstart sieht im Ereignisprotokoll genauso aus (1074 winlogon, 7002/7001, kein Boot-Ereignis). **Regel:** lange Läufe ohne Schlüssel und Netz mit `/RU <Benutzer> /NP` anlegen („ausführen, unabhängig von der Benutzeranmeldung", ohne Kennwort, nur lokale Ressourcen — node muss im Maschinen-PATH stehen); Läufe mit Schlüsseln (Nachlauf) bleiben interaktiv, weil `/NP` das Benutzerprofil nicht lädt. Vor jeder Aussage „der Lauf läuft" die Prozessliste und die Sicherungspunkte prüfen, nicht das Wiki.
