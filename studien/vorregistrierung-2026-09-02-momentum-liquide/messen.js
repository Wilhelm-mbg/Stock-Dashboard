'use strict';
/* MONATS-MOMENTUM IN LIQUIDER FASSUNG MESSEN. Kopie von studien/vorregistrierung-2026-09-02-
 * momentum-messung/messen.js (Stand 3e7d17c), erweitert um GENAU EINEN Parameter:
 *
 *   --umsatzMin=<$>   Korbfilter VOR der Rangbildung: ein Wert ist in Periode t nur
 *                     zulaessig, wenn sein Median-Tagesumsatz (Schluss x Stueck) ueber die
 *                     20 Balken bis einschliesslich t >= umsatzMin ist. Punkt-in-Zeit, aus
 *                     den Balken bis t - nie mit heutigen Werten. Dieselbe Rechnung wie das
 *                     Liquiditaetsprofil des Vorbilds (periode()), kein Nachbau.
 *                     0 = Filter aus = exakt das alte Verhalten.
 *
 * Werkzeug zur VORREGISTRIERUNG.md in diesem Ordner; gebaut NACH deren Commit (51ac880)
 * und committet, BEVOR es mit Filter auf Ertraege laeuft. Der Vorbildordner bleibt unveraendert.
 *
 * WAS SONST GLEICH IST: Anordnung 231/21/63, staerkste 10 %, nicht ueberlappend, Phase 0,
 * Schnitt 2006-08-14, Urteil nur Bestaetigung, Kosten K + F x Naechte je Gefaess, Placebo
 * (Saat 20260902, 200 Ziehungen), Positivkontrolle x1,02, #85, Delisting-Ausstieg, Pruefung
 * der Ueberlebensverzerrung. Vergleichsgruppe ist der gleichgewichtete Mittelwert ALLER
 * ZULAESSIGEN - mit Filter also der liquiden - Werte derselben Periode (Registrierung §3).
 *
 * WAS NEU IST (Registrierung §5/§6):
 *   - W0: in JEDEM Lauf zuerst ein Durchgang OHNE Filter; muss den Vorbildwert +1,54097 Pp
 *     (79 Perioden) auf < 0,01 reproduzieren, sonst Halt.
 *   - Urteil LEBT / STIRBT / nicht entscheidbar auf dem BRUTTO-Effekt des liquiden Korbs,
 *     Hilfsgroesse gepaartes Delta = brutto_liq - brutto_breit je Periode.
 *   - Familie 6 Tests (|t| >= 2,638) fuer die Gefaess-Zeilen (nachrichtlich).
 *   - Im Modus --waechter wird der Mittelwert des liquiden Korbs NICHT gedruckt (nur se,
 *     Korbgroessen, Kontrollen).
 *
 * Aufruf:
 *   node messen.js --waechter --umsatzMin=100000000
 *   node messen.js --lauf     --umsatzMin=100000000     Rohdaten nach lauf-*.json
 */
...
/* Schwellen. Familie momentum: 4 Maschinen-Varianten + Vorbildmessung + dieser Lauf = 6 Tests
 * -> Bonferroni zweiseitig 0,05/6, |t| >= 2,638 (Registrierung §5). */
var T_FAM = 2.6383, Z_NOM = 1.959964, Z80 = 0.8416212;

/* Kontrollen. */
var SEED = 20260902, PLACEBO_ZIEHUNGEN = 200, IMPLANTAT = 0.02;
/* W0: Vorbildwert (lauf-2026-09-01-22-25.json, urteil.bruttoBestaetigung.mittel), Filter aus. */
var VORBILD = { brutto: 1.5409705140566468, perioden: 79, toleranz: 0.01,
                datei: path.join(REPO, 'studien', 'vorregistrierung-2026-09-02-momentum-messung', 'lauf-2026-09-01-22-25.json') };
var BREIT_REFERENZ = 1.541;                      /* Bezugswert der Registrierung §5 */
var HAELFTE = 0.770;                             /* 50 % von 1,541 - Bezugsgroesse "die Haelfte", §5 */

var arg = {};
process.argv.slice(2).forEach(function (a) { var m = /^--([a-zA-Z0-9]+)=?(.*)$/.exec(a); if (m) arg[m[1]] = m[2] || true; });
var MODUS = arg.lauf ? 'lauf' : (arg.waechter ? 'waechter' : null);
if (!MODUS) { console.error('Bitte --waechter oder --lauf angeben.'); process.exit(2); }
/* Der einzige neue Parameter. Vorab gesetzt (100 Mio $), nicht gesucht, nicht variiert. */
var UMSATZ_MIN = arg.umsatzMin == null || arg.umsatzMin === true ? 0 : Number(arg.umsatzMin);
if (!(UMSATZ_MIN >= 0)) { console.error('--umsatzMin muss eine Zahl >= 0 sein.'); process.exit(2); }
 ...
 * umsatzMin > 0: Werte mit Median-Tagesumsatz < umsatzMin sind NICHT zulaessig (weder Korb
 * noch Vergleich) - der einzige Eingriff dieser Studie, VOR korbBilden(). */
function periode(reihen, syms, startTag, endTag, umsatzMin) {
  var kand = [];
  umsatzMin = umsatzMin || 0;
  for (var si = 0; si < syms.length; si++) {
    ...
    var p1 = b[iE][1];
    if (!(p1 > 0)) continue;
    /* Liquiditaet: Median Schluss x Stueck ueber die 20 Balken bis i - Punkt-in-Zeit. Im
     * Vorbild nur fuer das Profil; hier zusaetzlich der Filter (Registrierung §3). */
    var ums = [];
    for (var q = Math.max(0, i - 19); q <= i; q++) ums.push((b[q][1] || 0) * (b[q][2] || 0));
    var umsatz = median(ums);
    if (umsatzMin > 0 && !(umsatz >= umsatzMin)) continue;
    kand.push({ sym: R.sym, staerke: m2 / a - 1, folge: p1 / p0 - 1, p0: p0, p1: p1,
                quelle: R.quelle, frueh: ausstiegFrueh, umsatz: umsatz });
  }
  return kand;
}
...
function laufe(reihen, syms, TAGE, phase, opts) {
  ...
  for (var t = MERKMAL_START + phase; t + HALTEN < TAGE.length; t += HALTEN) {
    ...
    var kand = periode(reihen, syms, startTag, endTag, opts.umsatzMin);
    if (kand.length < MINDEST_WERTE) { if (opts.ausgefallen) opts.ausgefallen.push({ tag: startTag, n: kand.length }); continue; }
...
/* ================= Ablauf ================= */
console.log('== Monats-Momentum LIQUIDE messen ==  Modus ' + MODUS + '  Saat ' + SEED + '  Familie 6 Tests, |t| >= ' + T_FAM);
console.log('Rueckblick ' + RUECKBLICK + ' Luecke ' + LUECKE + ' Halten ' + HALTEN + ' Anteil ' + (ANTEIL * 100) + ' %  Schnitt ' + SCHNITT + '  Phase 0');
console.log('KORBFILTER: Median-Tagesumsatz (20 Balken bis t) >= ' + UMSATZ_MIN.toLocaleString('de-DE') + ' $' + (UMSATZ_MIN > 0 ? '' : '   ** FILTER AUS - reines Vorbild-Verhalten **'));
...
/* ---------- W0: Durchgang OHNE Filter - muss das Vorbild reproduzieren ---------- */
console.log('\n-- W0 Reproduktion des Vorbilds (Filter AUS, Bestaetigung, Phase 0, brutto) --');
var alleBreit = laufe(UE.reihen, symsS, TAGE, 0, { detail: false, umsatzMin: 0 });
var bestBreit = alleBreit.filter(function (P) { return P.hf === 'bestaetigung'; });
var stW0 = statistik(bestBreit.map(function (P) { return P.brutto; }));
if (!stW0) { console.error('W0: zu wenige Perioden.'); process.exit(1); }
var w0abw = Math.abs(stW0.mittel - VORBILD.brutto);
var w0ok = w0abw < VORBILD.toleranz && stW0.n === VORBILD.perioden;
console.log('  Vorbild +' + VORBILD.brutto.toFixed(5) + ' Pp / ' + VORBILD.perioden + ' Perioden   hier ' + pp(stW0.mittel, 5) + ' Pp / ' + stW0.n + ' Perioden   Abweichung ' + w0abw.toFixed(5) + '  -> ' + (w0ok ? 'BESTANDEN' : 'VERFEHLT - Umbau hat etwas veraendert, nichts darf gemessen werden'));
/* Nachrichtlich: groesste Abweichung je Periode gegen die Rohdaten des Vorbilds. */
var w0maxJePeriode = null, w0fehlend = null;
try {
  var vj = JSON.parse(fs.readFileSync(VORBILD.datei, 'utf8'));
  var vmap = {}; vj.perioden.forEach(function (P) { vmap[P.tag] = P.brutto; });
  var mx = 0, fehl = 0;
  bestBreit.forEach(function (P) { if (vmap[P.tag] == null) { fehl++; return; } var d = Math.abs(P.brutto - vmap[P.tag]); if (d > mx) mx = d; });
  w0maxJePeriode = mx; w0fehlend = fehl;
  console.log('  Groesste Abweichung je Periode gegen ' + path.basename(VORBILD.datei) + ': ' + mx.toExponential(2) + ' Pp   (Perioden ohne Gegenstueck: ' + fehl + ')');
} catch (e) { console.log('  Vorbild-Rohdaten nicht lesbar (' + (e.code || e) + ') - Periodenvergleich entfaellt.'); }

/* ---------- Phase 0 MIT Filter, beide Haelften, mit Detail ---------- */
var ausgefallen = [];
var alle0 = laufe(UE.reihen, symsS, TAGE, 0, { detail: true, umsatzMin: UMSATZ_MIN, ausgefallen: ausgefallen });
...
var korbN = best.map(function (P) { return P.korbN; }), breiteN = best.map(function (P) { return P.n; });
console.log('\n-- Liquider Korb (Bestaetigung, Phase 0): Groessen und Streuung - OHNE Mittelwert im Waechtermodus --');
console.log('  Perioden ' + stB.n + ' (Vorbild breit ' + bestBreit.length + ')   an Mindestbreite ' + MINDEST_WERTE + ' gescheitert: ' + ausgefallen.filter(function (a) { return a.tag >= SCHNITT; }).length + ' (Bestaetigung) / ' + ausgefallen.length + ' (gesamt)');
console.log('  Liquides Universum je Periode: min ' + Math.min.apply(null, breiteN) + '  Median ' + median(breiteN) + '  max ' + Math.max.apply(null, breiteN) + '   Korb: min ' + Math.min.apply(null, korbN) + '  Median ' + median(korbN) + '  max ' + Math.max.apply(null, korbN));
console.log('  Streuung je Periode sd ' + fx(stB.sd, 3) + ' Pp   se ' + fx(stB.se, 3) + ' Pp   (breit: sd ' + fx(stW0.sd, 3) + ', se ' + fx(stW0.se, 3) + ')   MDE 2se ' + fx(2 * stB.se, 3) + '   delta80 ' + fx(stB.delta80, 3));
console.log('  Delisting-Ausstiege im liquiden Korb (Bestaetigung): ' + fruehe);
...
console.log('  Kriterium des Vorbilds, NUR NACHRICHTLICH (sd Placebo-Mittel / se Kandidat): ' + fx(verhRB, 3) + '  (misst Faktorstreuung, nicht registriert)');
console.log('  Kriterium REGISTRIERT (se Placebo / sd Placebo-Mittel, Nachtrag 1 Vorbild): ' + fx(verhEigen, 3) + '  -> ' + (placOk ? 'BESTANDEN' : 'VERFEHLT'));
...
var kontrollen = { w0: { ok: w0ok, vorbild: VORBILD.brutto, hier: stW0.mittel, n: stW0.n, abweichung: w0abw, toleranz: VORBILD.toleranz, maxAbweichungJePeriode: w0maxJePeriode, periodenOhneGegenstueck: w0fehlend },
                   liquideVorab: { perioden: stB.n, sd: stB.sd, se: stB.se, mde: 2 * stB.se, delta80: stB.delta80, korbMin: Math.min.apply(null, korbN), korbMedian: median(korbN), korbMax: Math.max.apply(null, korbN),
                                   universumMin: Math.min.apply(null, breiteN), universumMedian: median(breiteN), universumMax: Math.max.apply(null, breiteN), ausgefallen: ausgefallen, delistingAusstiege: fruehe },
                   placebo: { ok: placOk, vorbildKriteriumNachrichtlich: placOkRegistriert, mittel: stP.mittel, se: stP.se, sd: stP.sd, t: stP.t, nettoCfd: stPnet.mittel, rauschbodenSd: sdRB,
                              verhaeltnisVorbildkriterium: verhRB, verhaeltnisRegistriert: verhEigen, ueberSchwelle: rbUeber, ziehungen: PLACEBO_ZIEHUNGEN },
...
if (MODUS === 'waechter') {
  fs.writeFileSync(path.join(HIER, 'waechter-' + new Date().toISOString().replace(/[:T]/g, '-').slice(0, 16) + '.json'), JSON.stringify({ modus: 'waechter', umsatzMin: UMSATZ_MIN, kontrollen: kontrollen, zaehl: UE.zaehl, zeitachse: { n: TAGE.length, von: TAGE[0], bis: TAGE[TAGE.length - 1] } }, null, 1));
  console.log('Waechter-Datei geschrieben. Kein Urteil in diesem Modus (Mittelwert des liquiden Korbs nicht gedruckt, nicht gespeichert).');
...
/* ---------- §5: gepaartes Delta liquide - breit (Paarung nach Starttag) ---------- */
var breitMap = {}; bestBreit.forEach(function (P) { breitMap[P.tag] = P; });
var paare = best.filter(function (P) { return breitMap[P.tag]; });
var deltaJe = paare.map(function (P) { return P.brutto - breitMap[P.tag].brutto; });
var stD = statistik(deltaJe);
/* Nachrichtlich: liquider Korb gegen den BREITEN Markt (Registrierung §8). */
var stGegenBreit = statistik(paare.map(function (P) { return P.korbRendite - breitMap[P.tag].marktRendite; }));

/* ---------- §5: LEBT / STIRBT / nicht entscheidbar ---------- */
function urteil5(b, d) {
  if (b.untereGrenze95 > 0 && b.mittel >= HAELFTE) return 'LEBT';
  if (b.obereGrenze95 < HAELFTE) return 'STIRBT (Weg 1: Obergrenze brutto liquide unter der Haelfte)';
  if (d && d.obereGrenze95 < -HAELFTE) return 'STIRBT (Weg 2: liquide nachweislich um mehr als die Haelfte schwaecher als breit)';
  return 'NICHT ENTSCHEIDBAR';
}
var U5 = urteil5(stB, stD);
/* Noetige Perioden fuer beide Richtungen bei diesem se (Formel wiki/aufloesungswand.md). */
var noetigLebt = stB.n * Math.pow(stB.delta80 / Math.max(HAELFTE, Math.abs(stB.mittel)), 2);
var noetigStirbtSe = (HAELFTE - Math.min(stB.mittel, HAELFTE - 1e-9)) / Z_NOM;   /* se, bei dem die Obergrenze die Haelfte unterschreitet (Punkt fest) */
var noetigStirbt = stB.mittel < HAELFTE ? stB.n * Math.pow(stB.se / noetigStirbtSe, 2) : Infinity;

console.log('\n================ URTEIL §5 - BRUTTO im liquiden Korb (Bestaetigung ' + best[0].tag + ' bis ' + best[best.length - 1].ende + ', Phase 0, ' + stB.n + ' unabhaengige Perioden) ================');
console.log('  brutto LIQUIDE ' + pp(stB.mittel) + ' Pp je Umlauf   sd ' + fx(stB.sd, 3) + '   se ' + fx(stB.se, 3) + '   t ' + fx(stB.t) + '   delta80 ' + fx(stB.delta80, 3) + ' (Familie ' + fx(stB.delta80Fam, 3) + ')');
console.log('  95-%-Band brutto liquide [' + pp(stB.untereGrenze95) + ', ' + pp(stB.obereGrenze95) + ']   Grenzen bei 2,638: [' + pp(stB.mittel - T_FAM * stB.se) + ', ' + pp(stB.obereGrenzeFam) + ']');
console.log('  brutto BREIT (W0, dieselben Perioden) ' + pp(stW0.mittel) + ' Pp   Bezug der Registrierung ' + pp(BREIT_REFERENZ) + '   die Haelfte ' + pp(HAELFTE) + '   Punkt liquide / breit = ' + fx(stB.mittel / stW0.mittel, 3));
console.log('  gepaartes Delta liquide - breit (' + stD.n + ' Paare): ' + pp(stD.mittel) + ' Pp   se ' + fx(stD.se, 3) + '   t ' + fx(stD.t) + '   95-%-Band [' + pp(stD.untereGrenze95) + ', ' + pp(stD.obereGrenze95) + ']');
console.log('  Regeln: LEBT = untere Grenze > 0 UND Punkt >= ' + HAELFTE + ' | STIRBT = Obergrenze brutto < ' + HAELFTE + ' ODER Obergrenze Delta < -' + HAELFTE + ' | sonst nicht entscheidbar');
console.log('  >>> ' + U5 + ' <<<');
if (U5 === 'NICHT ENTSCHEIDBAR') console.log('  Noetige Perioden bei diesem se: fuer LEBT bei Kante max(Haelfte, Punkt) ~ ' + fx(noetigLebt, 0) + ' (' + fx(noetigLebt / 4, 0) + ' Jahre); fuer STIRBT Weg 1 bei festem Punkt ~ ' + (isFinite(noetigStirbt) ? fx(noetigStirbt, 0) + ' (' + fx(noetigStirbt / 4, 0) + ' Jahre)' : 'unerreichbar (Punkt >= Haelfte)'));
console.log('  Placebo in derselben Zeile: brutto ' + pp(stP.mittel) + ' Pp, se ' + fx(stP.se, 3) + ', t ' + fx(stP.t));
console.log('  Nachrichtlich: liquider Korb gegen BREITEN Markt ' + pp(stGegenBreit.mittel) + ' Pp (se ' + fx(stGegenBreit.se, 3) + ', t ' + fx(stGegenBreit.t) + ') - Wahl der Vergleichsgruppe, kein Urteil');

console.log('\n-- §5b Gefaesse im liquiden Korb - NACHRICHTLICH, dieselben Regeln wie das Vorbild, Familie 2,638 --');
...
zeile('CFD gehebelt   (nachrichtl.) ', G.cfd, 'kein Urteil dieser Studie - Vorbild: nicht entscheidbar');
...
console.log('\n-- Nachrichtlich: Umschlag, Korbgroesse und Liquiditaetsprofil des LIQUIDEN Korbs (Bestaetigung) --');
...
console.log('  Korb im Mittel ' + fx(korbGroesse, 0) + ' von ' + fx(breite, 0) + ' liquiden Werten (breit: ' + fx(mittel(bestBreit.map(function (P) { return P.korbN; })), 0) + ' von ' + fx(mittel(bestBreit.map(function (P) { return P.n; })), 0) + '); gehalten aus der Vorperiode ' + fx(100 * mittel(umschl), 1) + ' %  -> effektives K bei Teil-Umschlag ' + fx(K * (1 - mittel(umschl)), 3) + ' Pp');
console.log('  Korbwerte mit Median-Tagesumsatz >= 1 Mrd $: ' + fx(100 * liq.ge1Mrd, 1) + ' %   >= 100 Mio $: ' + fx(100 * liq.ge100Mio, 1) + ' % (per Bau)   < 5 Mio $: ' + fx(100 * liq.unter5Mio, 1) + ' % (per Bau 0)');
var jahreLiq = {};
best.forEach(function (P) { var j = P.tag.slice(0, 4); (jahreLiq[j] || (jahreLiq[j] = { n: [], k: [] })); jahreLiq[j].n.push(P.n); jahreLiq[j].k.push(P.korbN); });
console.log('  Liquides Universum / Korb je Jahr (Mittel): ' + Object.keys(jahreLiq).sort().map(function (j) { return j + ' ' + fx(mittel(jahreLiq[j].n), 0) + '/' + fx(mittel(jahreLiq[j].k), 0); }).join('  '));

console.log('\n-- Nachrichtlich: alle 63 Rasterlagen (Bestaetigung, liquide) - Streubild, KEINE Tests --');
var lagen = [];
for (var ph = 0; ph < HALTEN; ph++) {
  var per = laufe(UE.reihen, symsS, TAGE, ph, { umsatzMin: UMSATZ_MIN }).filter(function (P) { return P.hf === 'bestaetigung'; });
...
if (symsV.length) {
  ...
  var vereinigung0 = laufe(reihenU, symsU, TAGE, 0, { detail: true, umsatzMin: UMSATZ_MIN });
  var vergleich = [];
  ...
  for (var ph2 = 0; ph2 < HALTEN; ph2++) {
    laufe(reihenU, symsU, TAGE, ph2, { detail: true, umsatzMin: UMSATZ_MIN }).forEach(function (P) {
...
var out = { studie: 'vorregistrierung-2026-09-02-momentum-liquide', modus: 'lauf', zeit: stempel, saat: SEED, umsatzMin: UMSATZ_MIN,
  parameter: { rueckblick: RUECKBLICK, luecke: LUECKE, halten: HALTEN, anteil: ANTEIL, mindestWerte: MINDEST_WERTE, schnitt: SCHNITT, phase: 0, umsatzMin: UMSATZ_MIN },
  kosten: { K: K, F: F, kassaAnnahme: KASSA, naechteMittel: naechteMittel, huerdeCfd: huerdeCfd },
  schwellen: { familie: T_FAM, tests: 6, nominal: Z_NOM, z80: Z80, breitReferenz: BREIT_REFERENZ, haelfte: HAELFTE },
  ...
  urteil5: { urteil: U5, bruttoLiquide: stB, bruttoBreitW0: stW0, deltaGepaart: stD, paare: stD.n, gegenBreitenMarkt: stGegenBreit,
             noetigePeriodenLebt: noetigLebt, noetigePeriodenStirbtWeg1: isFinite(noetigStirbt) ? noetigStirbt : null,
             verhaeltnisPunkt: stB.mittel / stW0.mittel },
  gefaesseNachrichtlich: { cfd: G.cfd, ungehebelt: G.ungehebelt, kassa: G.kassa },
  aeren: aeren, umschlagGehalten: mittel(umschl), liquiditaet: liq, korbGroesse: korbGroesse, breite: breite, jeJahr: jahreLiq, ausgefallen: ausgefallen,
  ...
  periodenBreitW0: bestBreit.map(function (P) { return { tag: P.tag, brutto: P.brutto, n: P.n, korbN: P.korbN, marktRendite: P.marktRendite }; }),
  deltaJePeriode: deltaJe, placeboJePeriode: plac };
