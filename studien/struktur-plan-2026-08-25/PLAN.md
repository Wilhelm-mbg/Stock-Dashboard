...
**Nachtrag 25.08. nachts: Stufe E ist KOMPLETT (7/7).** Alle sieben Blöcke sind wortgetreu ausgelagert: `wendeui.js`, `strategiechart.js`, `berichte.js` (Retro/Wochenreport/Analyse-Export/Messbericht), `backfill.js`, `btpool.js` + `backtestui.js`, `zucht.js`, `kosten.js` — depot.js von 9.998 auf ~6.900 Zeilen. Muster überall gleich: byte-genauer Skript-Schnitt, `verkabeln(deps)` mit benannten Abhängigkeiten, neu zuweisbare Zustände (D, SENT, EXPORT_ABDECKUNG) als **Getter**, Handelspfad-Aufrufe über **Aliase** wörtlich unverändert, Testmarken auf die neuen Dateien umgehängt (mit Auffindbarkeits-Prüfungen vor stillen indexOf-Scheiben), je Block dreifach verifiziert (test-v6, UI-Probe, Funktionstest der isolierten Instanz) und einzeln gepusht. Bewusst NICHT angefasst: `intradayScan()`, der Autopilot-/Edge-Ring und `init()`-Drittelung — das bleibt das nächste eigene Vorhaben (E-Rest), zusammen mit Stufe F.

**Nachtrag 26.08. früh: der E-Rest ist geliefert, soweit er Struktur ist.** `init()` ist gedrittelt: die Einmal-Migrationen wohnen in `depotmigration.js` (laufen(D, deps), mutiert D, idempotent über Einzelflags — auf einem Bestand mit Abbruch mittendrin verifiziert), die ~18 Dauer-Takte stehen gebündelt in einem markierten Abschnitt am init-Ende, und der `render()`-Speicher-Nebeneffekt ist ein eigener Puls (`equityPuls` — die Depotkurve wächst jetzt auch ohne Reiterbesuch). Zwei Lehren aus dem Funktionstest: die eslint-Globals führen `Q`/`U` als Fenster-Globals, obwohl nur `Quant`/`U` existieren — ein ReferenceError rutschte so durch Linter UND Textmarken und wurde erst vom Test der isolierten Instanz gefangen (eslint-Konfig-Befund, offen); die UI-Probe prüft seither auch das Warnband auf init-Abbruch. **Damit bleiben als bewusste Ausnahmen nur noch:** `intradayScan()` und der Autopilot-/Edge-Ring (Handelscode — nur mit Wilhelms Einzelentscheid), Stufe F und die Konzeptfragen 1/2/5.
