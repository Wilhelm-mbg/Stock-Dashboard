...
pos.forEach(p => {
  ...
  a.n++; a.h60=(a.h60||0)+p.richtung*(b[r+60][1]/b[r][1]-1); a.h60ohne1=(a.h60ohne1||0)+p.richtung*(b[r+60][1]/b[r+1][1]-1); a.absVor += Math.abs(vor); a.absNach += Math.abs(nach); a.sigVor += p.richtung * vor; a.sigNach += p.richtung * nach;
});
console.log('Klasse            n   |ret| r-1->r   |ret| r->r+1   Richtung*ret r-1->r   Richtung*ret r->r+1');
Object.keys(agg).forEach(k => { const a = agg[k]; const f = x => (100 * x / a.n).toFixed(2).padStart(8) + ' %'; console.log(k.padEnd(14), '60T roh', (100*a.h60/a.n).toFixed(2)+'%', 'ohne Tag1', (100*a.h60ohne1/a.n).toFixed(2)+'%  |', String(a.n).padStart(6), f(a.absVor), '   ', f(a.absNach), '      ', f(a.sigVor), '          ', f(a.sigNach)); });
